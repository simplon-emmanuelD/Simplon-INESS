...
class GB_JSON_API_Response {
  ...
  function setup() {
    ...
    if ($gb_json_api->query->exclude) {
      $this->exclude_values = explode(',', $gb_json_api->query->exclude);
  ...
  function respond($result, $status = 'ok') {
    ...
    } else if ($gb_json_api->query->redirect) {
      $url = $this->add_status_query_var($gb_json_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($gb_json_api->query->callback) {
