...
class GB_JSON_API_Post {
  ...
  function GB_JSON_API_Post($wp_post = null) {
    if (!empty($wp_post)) {
  ...
  function save($values = null) {
    ...
    if (!empty($values['author'])) {
      $author = $gb_json_api->introspector->get_author_by_login($values['author']);
    ...
    if (isset($values['categories'])) {
      ...
      foreach ($categories as $category_slug) {
        ...
        $category = $gb_json_api->introspector->get_category_by_slug($category_slug);
    ...
    if (!empty($_FILES['attachment'])) {
      ...
      $this->attachments[] = new GB_JSON_API_Attachment($attachment_id);
  ...
  function set_value($key, $value) {
    global $gb_json_api;
    if ($gb_json_api->include_value($key)) {
      $this->$key = $value;
    } else {
      unset($this->$key);
    }
  ...
  function set_categories_value() {
    global $gb_json_api;
    if ($gb_json_api->include_value('categories')) {
      $this->categories = array();
      if ($wp_categories = get_the_category($this->id)) {
        foreach ($wp_categories as $wp_category) {
          $category = new GB_JSON_API_Category($wp_category);
  ...
  function set_tags_value() {
    global $gb_json_api;
    if ($gb_json_api->include_value('tags')) {
      $this->tags = array();
      if ($wp_tags = get_the_tags($this->id)) {
        foreach ($wp_tags as $wp_tag) {
          $this->tags[] = new GB_JSON_API_Tag($wp_tag);
        }
      }
    } else {
      unset($this->tags);
  ...
  function set_author_value($author_id) {
    global $gb_json_api;
    if ($gb_json_api->include_value('author')) {
      $this->author = new GB_JSON_API_Author($author_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function set_comments_value() {
    global $gb_json_api;
    if ($gb_json_api->include_value('comments')) {
      $this->comments = $gb_json_api->introspector->get_comments($this->id);
    } else {
      unset($this->comments);
    }
  }
  ...
  function set_attachments_value() {
    global $gb_json_api;
    if ($gb_json_api->include_value('attachments')) {
      $this->attachments = $gb_json_api->introspector->get_attachments($this->id);
    } else {
  ...
  function set_thumbnail_value() {
    global $gb_json_api;
    if (!$gb_json_api->include_value('thumbnail') ||
  ...
  function get_thumbnail_size() {
    global $gb_json_api;
    if ($gb_json_api->query->thumbnail_size) {
      return $gb_json_api->query->thumbnail_size;
