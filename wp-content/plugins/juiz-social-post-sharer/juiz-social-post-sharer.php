			...
			if ( $show_me ) {
				...
				foreach( $juiz_sps_networks as $k => $v ) {
					if ( $v[0] == 1 ) {
						...
						$juiz_sps_content .= '<' . $li . ' class="juiz_sps_item juiz_sps_link_' . esc_attr( $k ) . '"><a href="' . wp_strip_all_tags( esc_attr( $api_link ) ) . '" ' . $rel_nofollow . '' . $more_attr . ' title="' . esc_attr( $api_text ) . '"' . $juiz_sps_target_link . '><span class="juiz_sps_icon jsps-' . esc_attr( $k ) . '"></span><span class="juiz_sps_network_name">' . esc_html( $network_name ) . '</span></a></' . $li . '>';
